...
func randomize_wander():
    if ant.random_move:
        if ant.move_direction != Vector2.ZERO:
            # Get the current direction's angle
            var current_angle = ant.move_direction.angle()
            ...
            ant.move_direction = Vector2(cos(current_angle + angle_offset), sin(current_angle + angle_offset)).normalized()
        else:
            # If the current direction is Vector2.ZERO, just pick a random direction as before
            ant.move_direction = Vector2(randf_range( - 1, 1), randf_range( - 1, 1)).normalized()
...
func Physics_Update(_delta: float) -> void:
    ...
    if ant:
        if ant.is_tasting:
            transitioned.emit(self, "AntInPheromone")
            return
        ant.lighten_sprite()
    ...
    # Randomize direction slightly to avoid getting stuck
        ...
        if ant.global_position.x < buffer:
            ant.move_direction.x = abs(ant.move_direction.x) # Ensure positive direction
            ant.global_position.x = buffer
        elif ant.global_position.x > screen_size.x - buffer:
            ant.move_direction.x = -abs(ant.move_direction.x) # Ensure negative direction
            ant.global_position.x = screen_size.x - buffer

        # Adjust for vertical screen bounds with buffer
        if ant.global_position.y < buffer:
            ant.move_direction.y = abs(ant.move_direction.y) # Ensure positive direction
            ant.global_position.y = buffer
        elif ant.global_position.y > screen_size.y - buffer:
            ant.move_direction.y = -abs(ant.move_direction.y) # Ensure negative direction
        ...
        ant.velocity = ant.move_direction.normalized() * move_speed
